...
grep -Fq -- '--memory-swap "$egress_memory_swap"' "$deploy_dir/scripts/run-egress.sh"
grep -Fq 'EXECUTOR_PROCESSOR_MEMORY_SWAP=6g' "$deploy_dir/config/host.env.example"
grep -Fq 'CUADRABOT_HOST_PROFILE=budget' "$deploy_dir/config/budget-host.env.example"
grep -Fq 'EXECUTOR_PROCESSOR_MEMORY_SWAP=1g' "$deploy_dir/config/budget-host.env.example"
grep -Fq 'EXECUTOR_EGRESS_MEMORY_SWAP=256m' "$deploy_dir/config/budget-host.env.example"
